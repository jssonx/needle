...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.mean(-np.log(np.exp(Z[np.arange(Z.shape[0]), y]) / np.sum(np.exp(Z), axis=1)))
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, y.shape[0], batch):
        ...
        I = np.eye(H.shape[1])[y_batch]
        gradient = 1/batch * np.dot(X_batch.T, Z - I)
        theta -= lr * gradient
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, y.shape[0], batch):
        ...
        Z1 = np.maximum(np.dot(X_batch, W1), 0)
        G2 = np.exp(np.dot(Z1, W2))/np.sum(np.exp(np.dot(Z1, W2)), axis=1, keepdims=True) - I
        G1 = np.multiply((Z1 > 0).astype(np.float32), np.dot(G2, W2.T))
        W1 -= lr/batch * np.dot(X_batch.T, G1)
